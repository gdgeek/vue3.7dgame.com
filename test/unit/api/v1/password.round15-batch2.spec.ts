...
vi.mock("@/environment", () => ({ default: { email_api: "https://email.example.com" } }));
...
describe("api/v1/password", () => {
  ...
  it("requestPasswordReset returns response data", async () => {
    ...
    await expect(api.requestPasswordReset("a@b.com")).resolves.toEqual({ success: true, message: "ok" });
  });
  ...
  it("verifyResetCode returns response data", async () => {
    ...
    await expect(api.verifyResetCode("a@b.com", "123456")).resolves.toEqual({ success: true, valid: true });
  });
  ...
  it("resetPasswordByCode returns response data", async () => {
    ...
    await expect(api.resetPasswordByCode("a@b.com", "123456", "P@ss")).resolves.toEqual({ success: true, message: "changed" });
  });
  ...
  it("changePassword propagates request failure", async () => {
    ...
    await expect(api.changePassword("old", "new", "new")).rejects.toThrow("change fail");
  });
});
