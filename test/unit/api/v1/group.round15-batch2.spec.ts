...
describe("api/v1/group round15 batch2", () => {
  ...
  it("getGroup includes expand", async () => {
    ...
    expect(request.mock.calls[0][0]).toEqual(
      expect.objectContaining({ method: "get", url: "/v1/group/9?expand=image" })
    );
  });

  it("create update patch delete use CRUD endpoints", async () => {
    await api.createGroup({ name: "g" } as Parameters<typeof api.createGroup>[0]);
    await api.updateGroup(1, { name: "u" } as Parameters<typeof api.updateGroup>[1]);
    await api.patchGroup(2, { description: "p" });
    await api.deleteGroup(3);
    expect(request.mock.calls[0][0]).toEqual(expect.objectContaining({ url: "/v1/group", method: "post" }));
    expect(request.mock.calls[1][0]).toEqual(expect.objectContaining({ url: "/v1/group/1", method: "put" }));
    expect(request.mock.calls[2][0]).toEqual(expect.objectContaining({ url: "/v1/group/2", method: "patch" }));
    expect(request.mock.calls[3][0]).toEqual(expect.objectContaining({ url: "/v1/group/3", method: "delete" }));
  });
  ...
  it("options join leave call action endpoints", async () => {
    ...
    expect(request.mock.calls[0][0]).toEqual(expect.objectContaining({ url: "/v1/group", method: "options" }));
    expect(request.mock.calls[1][0]).toEqual(expect.objectContaining({ url: "/v1/group/7/join", method: "post" }));
    expect(request.mock.calls[2][0]).toEqual(expect.objectContaining({ url: "/v1/group/7/leave", method: "post" }));
  });

  it("createGroupVerse posts nested endpoint", async () => {
    await api.createGroupVerse(8, { title: "v" } as Parameters<typeof api.createGroupVerse>[1]);
    expect(request.mock.calls[0][0]).toEqual(
  ...
  it("deleteGroupVerse uses nested delete endpoint", async () => {
    ...
    expect(request).toHaveBeenCalledWith({ url: "/v1/group/2/verse/99", method: "delete" });
  });
});
