...
import type { SceneWorkspaceContext } from "@/services/webmcp/scene-workspace-tools";

const CONTEXT = "xrugc_get_scene_workspace_context";
const OPEN = "xrugc_open_scene_script_editor";
const CLOSE = "xrugc_close_scene_script_editor";
...
function mountWorkspace() {
  ...
  const context = (): SceneWorkspaceContext => ({
    sceneId: ownerId.value,
    sceneName: `Scene ${ownerId.value}`,
    activeEditor: scriptOpen.value ? "scene-script" : "scene",
...
describe("scene workspace WebMCP lifecycle", () => {
  it("keeps navigation tools registered across scene → script drawer → scene on the same route", async () => {
    const { registry, scriptDirty } = mountWorkspace();
    const navigation = [CONTEXT, OPEN, CLOSE].map((name) => registry.get(name));
    expect(registry.has("xrugc_get_scene_editor_context")).toBe(true);
    await expect(registry.get(OPEN)!.execute({})).resolves.toMatchObject({
      status: "opened",
      context: { activeEditor: "scene-script" },
    });
    expect(registry.has("xrugc_get_scene_editor_context")).toBe(false);
    expect(registry.has("xrugc_get_scene_script")).toBe(true);
    ...
    await expect(registry.get(CLOSE)!.execute({})).resolves.toMatchObject({
      status: "closed",
      context: { activeEditor: "scene" },
    });
    expect(registry.has("xrugc_get_scene_script")).toBe(false);
    expect(registry.has("xrugc_get_scene_editor_context")).toBe(true);
  ...
  it("rejects an old owner's pending open before it can open the new owner's drawer", async () => {
    ...
    ownerId.value = 2329;
    const failed = expect(pending).rejects.toThrow("场景工作区已切换");
    gate.resolve();
    await failed;
