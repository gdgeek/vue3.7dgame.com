...
import enUS from "@/lang/en-US/homepage";

describe("en-US homepage lang", () => {
  it("has a homepage key", () => {
    expect(enUS).toHaveProperty("homepage");
  });

  it("greeting keys are non-empty English strings", () => {
    const { greeting } = enUS.homepage;
  ...
  it("myCreation section has required keys", () => {
    const mc = enUS.homepage.myCreation;
    for (const key of [
      "title",
      "myPolygen",
      "myPicture",
  ...
  it("quickStart section has upload/edit/create sub-sections", () => {
    const qs = enUS.homepage.quickStart;
  ...
  it("edit section personalData key exists and is a string", () => {
    expect(typeof enUS.homepage.edit.personalData).toBe("string");
    expect(enUS.homepage.edit.personalData.length).toBeGreaterThan(0);
  });

  it("account section title key is a non-empty string", () => {
    expect(typeof enUS.homepage.account.title).toBe("string");
    expect(enUS.homepage.account.title.length).toBeGreaterThan(0);
  });

  it("announcements title is a non-empty string", () => {
    expect(typeof enUS.homepage.announcements.title).toBe("string");
    expect(enUS.homepage.announcements.title.length).toBeGreaterThan(0);
  });

  it("header subtitle is a non-empty string", () => {
    expect(typeof enUS.homepage.header.subtitle).toBe("string");
    expect(enUS.homepage.header.subtitle.length).toBeGreaterThan(0);
  });
});
