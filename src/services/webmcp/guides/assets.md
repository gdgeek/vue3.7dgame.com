...
多个素材优先按当前 WebMCP schema 搜索、绑定和添加；有批量工具则批量，没有则逐项执行，工具缺失时再用原生多选。区分资源绑定和节点部署，已有节点不要再次部署。完成后核对新增节点数量、名称、资源引用和保存回执；计数滞后时先确认保存再重读，不重复添加。

外部制作模型时先确认 BlenderMCP 实际连接，读取场景后仅修改目标集合并另存母版。核对 GLB 尺寸、轴向、边界、动画、材质、扩展和外部文件引用；检查器结果不证明蒙皮、画质或目标导入器兼容。网站不安装 Blender 或执行客户端辅助脚本。
