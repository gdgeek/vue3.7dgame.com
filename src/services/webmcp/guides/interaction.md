...
3. 实体逻辑进入实体脚本页；场景逻辑在场景工作区调用 `xrugc_open_scene_script_editor` 打开右侧脚本抽屉。`opened` 只表示打开，需再读常驻 `xrugc_get_scene_workspace_context` 确认 `script.ready`，即使 URL 不变也重新发现工具。用 `xrugc_get_meta_script` 或 `xrugc_get_scene_script` 读取工作区，并按需设置 `includeGeneratedCode: true` 查看真实 Lua/JavaScript。独立 `/verse/script` 仍可使用。
...
6. 抽屉内场景主工具暂停。返回场景调用 `xrugc_close_scene_script_editor`，按原未保存确认选择；工具不自动保存或放弃，`cancelled` 时保留抽屉。关闭成功后脚本工具注销，再读 `scene.ready` 并重新发现场景工具，才能开始场景运行。
