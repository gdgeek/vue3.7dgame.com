...
1. 工具缺失时检查浏览器支持、当前登录页面和编辑器是否加载完成。切页后重新发现工具；单独启动主前端不代表 Editor、Blockly、后端或内置运行制品已就绪。无法发现时使用可用的原生界面，不猜工具名。
...
7. 场景加载异常时用 `xrugc_get_scene_runtime_preview_status` 和场景编辑页的 `xrugc_get_scene_runtime_diagnostics` 区分加载器、Unity 启动、场景转发及资源错误。从场景脚本页转到编辑页后需重新发现诊断工具。
