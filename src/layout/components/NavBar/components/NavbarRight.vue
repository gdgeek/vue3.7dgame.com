<template>
  <div class="flex">
    <template v-if="!isMobile">
      ...
      <!-- 语言选择 -->
      <lang-select v-if="!domainStore.isLanguageLocked" class="setting-item"></lang-select>
    </template>
    <div>
      <!-- 或者独立使用它，不从父级获取属性 -->
    </div>
...
watch(
  ...
  (newUserInfo) => {
    ...
    if (
      newUserInfo.id !== null &&
      newUserInfo.id !== 0 &&
      newUserInfo.userData !== null
    ) {
      nickname.value =
        newUserInfo.userData.nickname ?? newUserInfo.userData.username;
...
const logout = async () => {
  try {
    await ElMessageBox.confirm(
      t("login.logout.message1"),
      t("login.logout.message2"),
      {
