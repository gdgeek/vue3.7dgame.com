...
function parseAction(node: any, parentUuid?: string) {
  if (!node || !node.parameters || typeof node.parameters.action === 'undefined') return null;
  return {
...
function parsePoint(node: any, typeList: string[]) {
  ...
  const match = typeList.find(t => node.type?.toLowerCase() === t.toLowerCase());
  ...
  return {
    ...
    ...(isPolygen ? { animations, moved: hasMoved, rotate: hasRotate, hasTooltips } : {}),
...
function walk(node: any, acc: MetaResourceIndex) {
  ...
  const entity = parsePoint(node, ['polygen','entity','voxel','video','picture','text','voxel','phototype','sound']);
  if (entity) acc.entity.push(entity);

  const polygen = parsePoint(node, ['polygen']); if (polygen) acc.polygen.push(polygen);
  const video = parsePoint(node, ['video']); if (video) acc.video.push(video);
  const picture = parsePoint(node, ['picture']); if (picture) acc.picture.push(picture);
  const sound = parsePoint(node, ['sound']); if (sound) acc.sound.push(sound);
  const text = parsePoint(node, ['text']); if (text) acc.text.push(text);
  const voxel = parsePoint(node, ['voxel']); if (voxel) acc.voxel.push(voxel);
  const phototype = parsePoint(node, ['phototype']); if (phototype) acc.phototype.push(phototype);
...
export function buildMetaResourceIndex(meta: metaInfo): MetaResourceIndex {
  ...
  const index: MetaResourceIndex = {
    action: [], trigger: [], polygen: [], picture: [], video: [], voxel: [], phototype: [], text: [], sound: [], entity: [],
