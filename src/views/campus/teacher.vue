  ...
  <TransitionWrapper>
    <div class="teacher-list">
      ...
      <ViewContainer class="list-view" :items="items" :view-mode="viewMode" :loading="loading" @row-click="openDetail">
        ...
        <template #list-item="{ item }">
          ...
          <div class="col-name">
            <div class="item-thumb">
              ...
              <div v-else class="thumb-placeholder"><span class="material-symbols-outlined">person_4</span></div>
            </div>
        ...
        <template #empty>
          <EmptyState icon="person_4" text="暂无教师" action-text="邀请教师" @action="addTeacher" />
        </template>
      </ViewContainer>
...
const {
  ...
  fetchFn: async (params) => await getTeachers(params.sort, params.search, params.page, "user,school,user.avatar"),
});
...
const detailProperties = computed(() => {
  ...
  return [
    { label: '教师姓名', value: currentTeacher.value.user?.nickname || currentTeacher.value.user?.username || '—' },
...
const handleDelete = async () => {
  if (!currentTeacher.value) return;
  try {
    await MessageBox.confirm("确定要移除该教师吗？", "移除确认", { type: "warning" });
