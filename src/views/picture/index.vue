...
const {
  ...
  fetchFn: async (params) => await getPictures(params.sort, params.search, params.page),
});
...
const detailProperties = computed(() => {
  ...
  const info = currentPicture.value.info ? JSON.parse(currentPicture.value.info) : null;
  return [
    { label: '类型', value: '图片' },
    { label: '大小', value: formatSize(currentPicture.value.file?.size) },
    { label: '创建时间', value: convertToLocalTime(currentPicture.value.created_at) },
...
const handleDownload = async () => {
  ...
  await downloadResource(
    { name: currentPicture.value.name || 'image', file: currentPicture.value.file },
    fileExt,
    t,
...
const savePicture = async (
  ...
  try {
    ...
    if (info) { data.info = info; data.image_id = file_id; }
...
const namedWindow = async (item: { id: string; name: string }) => {
  try {
    const { value } = (await MessageBox.prompt(
      t("picture.prompt.message1"), t("picture.prompt.message2"),
      { confirmButtonText: t("picture.prompt.confirm"), cancelButtonText: t("picture.prompt.cancel"), defaultValue: item.name }
    )) as { value: string };
    await putPicture(item.id, { name: value });
    refresh();
    Message.success(t("picture.prompt.success") + value);
  } catch { Message.info(t("picture.prompt.info")); }
};

const deletedWindow = async (item: { id: string }, resetLoading: () => void) => {
  try {
    await MessageBox.confirm(
      t("picture.confirm.message1"), t("picture.confirm.message2"),
      { confirmButtonText: t("picture.confirm.confirm"), cancelButtonText: t("picture.confirm.cancel"), type: "warning" }
    );
    await deletePicture(item.id);
    refresh();
    Message.success(t("picture.confirm.success"));
  } catch { Message.info(t("picture.confirm.info")); resetLoading(); }
};
