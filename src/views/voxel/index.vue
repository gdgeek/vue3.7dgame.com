        ...
        <template #list-item="{ item }">
          <div class="col-name">
            <div class="item-thumb">
              ...
              <div v-else class="thumb-placeholder"><span class="material-symbols-outlined">deployed_code</span></div>
            </div>
          ...
          <div class="col-actions">
            <el-dropdown trigger="click">
              ...
              <template #dropdown>
                <el-dropdown-menu>
                  <el-dropdown-item><router-link :to="`/resource/voxel/view?id=${item.id}`">{{ $t("voxel.viewVoxel")
...
import { PageActionBar, ViewContainer, PagePagination } from "@/components/StandardPage";
...
const {
  ...
  fetchFn: async (params) => await getVoxels(params.sort, params.search, params.page),
});
...
const saveVoxel = async (
  ...
  } catch (err) { console.error("Failed to save voxel:", err); callback(-1); }
};

const namedWindow = async (item: { id: string; name: string }) => {
  try {
    const { value } = (await ElMessageBox.prompt(t("voxel.prompt.message1"), t("voxel.prompt.message2"),
      { confirmButtonText: t("voxel.prompt.confirm"), cancelButtonText: t("voxel.prompt.cancel"), closeOnClickModal: false, inputValue: item.name }
    )) as { value: string };
    await putVoxel(item.id, { name: value }); refresh(); ElMessage.success(t("voxel.prompt.success") + value);
  } catch { ElMessage.info(t("voxel.prompt.info")); }
};

const deletedWindow = async (item: { id: string }, resetLoading: () => void) => {
  try {
    await ElMessageBox.confirm(t("voxel.confirm.message1"), t("voxel.confirm.message2"),
      { confirmButtonText: t("voxel.confirm.confirm"), cancelButtonText: t("voxel.confirm.cancel"), closeOnClickModal: false, type: "warning" });
    await deleteVoxel(item.id); refresh(); ElMessage.success(t("voxel.confirm.success"));
  } catch { ElMessage.info(t("voxel.confirm.info")); resetLoading(); }
};
