        ...
        <template #list-item="{ item }">
          <div class="col-name">
            <div class="item-thumb">
              ...
              <div v-else class="thumb-placeholder"><span class="material-symbols-outlined">blur_on</span></div>
            </div>
...
const {
  ...
  fetchFn: async (params) => await getParticles(params.sort, params.search, params.page),
});
...
const saveParticle = async (
  ...
  } catch (err) { console.error("Failed to save particle:", err); callback(-1); }
};

const namedWindow = async (item: { id: string; name: string }) => {
  try {
    const { value } = (await MessageBox.prompt(t("particle.prompt.message1"), t("particle.prompt.message2"),
      { confirmButtonText: t("particle.prompt.confirm"), cancelButtonText: t("particle.prompt.cancel"), defaultValue: item.name }
    )) as { value: string };
    await putParticle(item.id, { name: value }); refresh(); Message.success(t("particle.prompt.success") + value);
  } catch { Message.info(t("particle.prompt.info")); }
};

const deletedWindow = async (item: { id: string }, resetLoading: () => void) => {
  try {
    await MessageBox.confirm(t("particle.confirm.message1"), t("particle.confirm.message2"),
      { confirmButtonText: t("particle.confirm.confirm"), cancelButtonText: t("particle.confirm.cancel"), type: "warning" });
    await deleteParticle(item.id); refresh(); Message.success(t("particle.confirm.success"));
  } catch { Message.info(t("particle.confirm.info")); resetLoading(); }
};
