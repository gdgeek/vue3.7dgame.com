<template>
  <TransitionWrapper>
    <CardListPage ref="cardListPageRef" :fetch-data="fetchVideos" wrapper-class="video-index" @refresh="handleRefresh">
      ...
      <template #card="{ item }">
        <mr-p-p-card :item="item" type="视频" color="#1abc9c" @named="namedWindow" @deleted="deletedWindow">
          <template #enter>
      ...
      <template #dialogs>
        ...
        <VideoDialog v-model="viewDialogVisible" :id="currentVideoId" :auto-play="autoPlay" @refresh="refreshList"
          @deleted="refreshList" />
      </template>
    </CardListPage>
  </TransitionWrapper>
