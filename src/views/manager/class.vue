...
const handleSelectUser = async (user: any) => {
  ...
  try {
    if (userSelectionMode.value === 'teacher') {
      ...
      await createTeacher({ user_id: user.id, class_id: currentClass.value.id });
...
const refreshTeachers = async () => {
  if (!currentClass.value) return;
  try {
    const response = await getClass(currentClass.value.id);
    // Fetch the class with expanded teachers
    const fullResponse = await getClasses(
      '-created_at',
      '',
      1,
      'eduTeachers.user',
    ...
    // Find the current class in the response
    const updatedClass = fullResponse.data.find((c: EduClass) => c.id === currentClass.value!.id);
    if (updatedClass) {
...
const refreshStudents = async () => {
  ...
  try {
    const response = await getClass(currentClass.value.id);
    // Fetch the class with expanded students
    const fullResponse = await getClasses(
      '-created_at',
      '',
      1,
