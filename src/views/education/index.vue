  ...
  <div class="app-container" :class="{ 'dark-theme': isDark }">
    ...
    <div class="sidebar-menu" :class="{ 'sidebar-visible': sidebarVisible, 'dark-theme': isDark }" v-if="isMobile">
      ...
      <div class="sidebar-items">
        ...
        <div class="sidebar-item" @click="openLoginDialog; toggleSidebar()">
          登录平台
        </div>
      </div>
    </div>
    ...
    <div class="content-container" ref="contentRef" :class="{ 'dark-theme': isDark }">
      <router-view v-if="!isIndex"></router-view>
      <home-view v-if="isIndex" />
...
// 导航菜单项
const navMenuItems = [
  { key: "home", label: "首页", path: "/education/index" },
  { key: "solutions", label: "教育解决方案", path: "/education/solutions" },
  { key: "scenes", label: "教育应用场景", path: "/education/scenes" },
];
...
// 自动滚动
const SCROLL_POSITION_KEY = 'education_scroll_position';
...
onMounted(() => {
  ...
  router.beforeEach((to, from) => {
    if (from.path.startsWith('/education')) {
      saveScrollPosition();
    }
    return true;
  });
...
.nav-container {
  ...
  .nav-right {
    ...
    .login-button {
      ...
      background: linear-gradient(90deg, #0396FF 0%, #ABDCFF 100%);
      border: none;
      transition: all 0.3s ease;
      ...
      &:hover {
        ...
        background: linear-gradient(90deg, #ABDCFF 0%, #0396FF 100%);
