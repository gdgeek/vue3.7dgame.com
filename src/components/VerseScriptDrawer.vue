<template>
  <el-drawer
    ...
    :title="`${$t('route.project.scriptEditor')}${title ? ` · ${title}` : ''}`"
  ...
  >
    <template #header="{ titleId, titleClass }">
      <h2 :id="titleId" :class="titleClass">
        {{ $t("route.project.scriptEditor") }}{{ title ? ` · ${title}` : "" }}
      </h2>
      <div
        class="script-drawer-actions"
        role="group"
        :aria-label="$t('route.project.scriptEditor')"
      >
        <el-tooltip :content="$t('verse.view.script.save')" placement="bottom">
          <el-button
            class="script-drawer-action script-drawer-action--save"
            v-bind="{ 'aria-label': $t('verse.view.script.save') }"
            :disabled="
        ...
        <el-tooltip
          :content="$t('verse.view.script.drawer.versions')"
          placement="bottom"
        >
          <el-button
            class="script-drawer-action"
            v-bind="{ 'aria-label': $t('verse.view.script.drawer.versions') }"
            :disabled="
              !scriptEditor ||
              scriptEditor.editorContentLoading ||
              scriptEditor.isSaving
...
const handleClosed = () => {
  // The host restores its scene tools before the close tool reports success.
  emit("closed");
  finishClosing?.(true);
  finishClosing = null;
  closing = null;
};
const getState = (): SceneWorkspaceContext["script"] => ({
