<template>
  <el-dialog :model-value="modelValue" :title="title || $t('common.selectUser')" width="900px"
    ...
    <div class="user-list" v-loading="loading">
      ...
      <el-empty v-if="!loading && users.length === 0" :description="$t('common.noUsers')" />
    </div>

    <div class="pagination-container">
      <el-pagination v-if="pagination.total > 0" :current-page="pagination.current" :page-size="pagination.size"
        :total="pagination.total" layout="prev, pager, next" @current-change="handlePageChange" background />
    </div>
  </el-dialog>
</template>
...
const fetchUsers = async () => {
  ...
  try {
    ...
    pagination.value.total = parseInt(headers['x-pagination-total-count'] || '0');
    pagination.value.current = parseInt(headers['x-pagination-current-page'] || '1');
