// 通用翻译
export default {
  common: {
    appUpdate: {
      available:
        "网站有新版本。请先保存或导出未保存内容、确认待处理操作并关闭运行场景，再手动刷新；刷新后重新发现 WebMCP 工具。",
      dismiss: "稍后",
    },
    recovery: {
      summary: "保存冲突：本地修改仍未保存。",
      open: "查看冲突与保留副本",
      boundary:
        "这里保留的是被服务器拒绝的提交副本，可能不含之后的编辑。预览最多显示 12,000 字符，导出包含完整提交副本。读取服务器不会覆盖编辑器或更新保存基线；请先导出，再核对最新版并重新编辑。",
      export: "导出被拒绝的提交",
      read: "只读查看服务器最新版",
      keep: "保留本地并继续编辑",
      local: "本次提交副本",
      server: "服务器当前内容（只读）",
      failed: "读取失败或权限已变化，未显示服务器内容。",
    },
    publicationHistory: {
      retention:
        "最多保留最近 {count} 份发布正文，超出后自动清理旧正文；当前发布不受影响。",
      expired:
        "该版本正文已按保留策略清理。请刷新列表选择仍保留的版本，无需再次发布。",
      from: "比较起始版本",
      to: "比较目标版本",
      compare: "比较发布版本",
      identical: "原始正文完全相同",
      different: "原始正文不同；以下为结构差异摘要",
      truncated: "差异达到上限，结果不完整；请分别导出核对。",
      export: "导出正文与资源引用",
      pending:
        "发布已成功，历史正文尚未核验。可在发布历史中重试读取，请勿重复发布。",
      title: "发布历史",
      boundary:
        "服务器保存的固定发布版本，与本机草稿独立。查看历史不会覆盖当前场景。",
      refresh: "刷新列表",
      capacity: "{count} 个版本 · 已用 {used} / {budget}",
      capacityWarning: "归档容量接近上限，请联系管理员规划容量。",
      empty: "暂无固定发布历史。旧发布不会被自动补录为历史原件。",
      date: "发布时间",
      language: "语言",
      actor: "发布人 ID",
      version: "版本",
      inspect: "读取并核验",
      more: "加载更多",
      detail: "版本详情",
      verified: "历史正文核验通过",
      resources:
        "只核验已归档正文；资源文件未复制，可用性尚未检查，也不保证能恢复编辑工程。",
      body: "查看固定正文",
      forbidden: "当前账号没有读取此场景历史的权限。",
      failed:
        "读取或核验未通过。可重试读取；不会再次发布或用当前快照代替历史。",
    },
    editorLoading: {
      progressLabel: "资源载入进度",
      count: "已完成 {completed} / {total} 项",
      workFailed: "载入未完成，请重新载入。",
      stages: {
        connecting: "正在连接编辑器",
        data: "正在读取场景／实体数据",
        initializing: "正在初始化编辑器",
        assets: "正在载入资源",
        finishing: "正在整理场景与编辑状态",
        ready: "载入完成",
        error: "载入失败",
        model: "正在载入模型",
        module: "正在载入场景实体",
        anchor: "正在载入锚点",
        space: "正在载入空间背景",
      },
      message: "编辑器正在载入…",
      hint: "载入完成后即可编辑、保存、运行或打开脚本。",
      failed: "编辑器载入未完成，请重试。",
      retry: "重新载入",
    },
    editorSave: {
      conflict:
        "服务器拒绝了版本冲突写入。本地修改仍未保存，请保留内容并核对服务器最新版本后再操作。",
      unverified:
        "保存结果尚未确认。本地修改已保留，请先查询原操作回执和服务器状态。",
      pending:
        "仍有未确认保存的修改，请先核对服务器版本；编辑器无新增修改不代表保存成功。",
    },
    confirm: "确认",
    cancel: "取消",
    selectUser: "选择用户",
    username: "用户名",
    nickname: "昵称",
    select: "选择",
    noUsers: "未找到用户",
    noData: "暂无数据",
    enter: "进入",
    back: "返回",
    open: "打开",
    add: "添加",
    name: "名称",
    description: "描述",
    edit: "编辑",
    rename: "重新命名",
    updateSuccess: "更新成功",
    createSuccess: "创建成功",
    createFailed: "创建失败",
    delete: "删除",
    deleteSuccess: "删除成功",
    deleteFailed: "删除失败",
    operationFailed: "操作失败",
    networkError: "网络错误，请检查您的网络连接",
    updateFailed: "更新失败",
    invalidData: "数据无效",
    button: {
      save: "保存",
    },
    scriptSaveConfirm: {
      title: "提示",
      message: "是否保存当前脚本",
      confirm: "是",
      cancel: "否",
    },
    sceneSaveConfirm: {
      title: "提示",
      message: "是否保存当前场景",
      confirm: "是",
      cancel: "否",
    },
    entitySaveConfirm: {
      title: "提示",
      message: "是否保存当前实体",
      confirm: "是",
      cancel: "否",
    },
    scriptDraft: {
      title: "版本历史",
      autoSave: "自动保存",
      interval: "保存间隔",
      minutes: "分钟",
      statusSaving: "保存中",
      statusSaved: "已保存",
      statusAutoSaved: "已自动保存",
      statusDirty: "待保存",
      versionList: "历史版本",
      empty: "暂无历史版本",
      clearHistory: "清空历史版本",
      restore: "回滚到此版本",
      restoreSuccess: "已恢复到选中版本，请确认后保存",
      autoSavedNotice: "已自动保存",
      autoTag: "自动保存",
      manualTag: "手动保存",
      emptySummary: "暂无内容",
      summaryAdded: "新增 {items}",
      summaryRemoved: "删除 {items}",
      summaryModified: "修改 {items}",
      entry: "版本历史",
    },
    unityPreview: {
      entry: "运行场景",
      title: "运行场景",
      helpClick: "先单击运行画面，使其获得焦点",
      helpRotate: "按住 Alt（Mac：⌥ Option）并用鼠标左键拖动，旋转视角",
      helpZoomPan: "滚轮或触控板双指滚动可缩放；鼠标右键拖动可平移",
      helpFullscreen: "全屏可获得更完整预览",
      fullscreen: "全屏运行场景",
      close: "关闭运行场景",
    },
    message: {
      createSuccess: "创建成功",
      saveSuccess: "保存成功",
      saveFailed: "保存失败",
    },
  },
  errorPage: {
    back: "返回",
    gifSource: "图片来源于",
    page: "Oops! 页面出错了",
    noPermission: "你没有权限访问该页面",
    contactLeader: "请联系你的上级或管理员",
    orGoTo: "或者去",
    backHome: "回到首页",
    justLook: "随便逛逛",
    clickToSee: "点击查看",
    justLookTitle: "随便逛逛",
  },
  request: {
    loginExpired: "登录过期，请重新登录",
    networkError: "网络错误，请检查您的网络连接",
    serverError: "服务器内部错误，请稍后再试",
    error404: "请求失败，未找到资源 (404)",
    unknownError: "未知错误",
  },
  emailVerification: {
    codeSent: "验证码已发送到您的邮箱",
    verifySuccess: "邮箱验证成功",
    sentToCurrentEmail: "验证码已发送到当前绑定邮箱",
    oldEmailVerified: "旧邮箱验证成功，请继续完成新邮箱绑定",
    unbindSuccess: "邮箱解绑成功",
  },
  sidebar: {
    home: "主页",
    resources: "素材库",
    model: "模型",
    voxel: "体素",
    picture: "图片",
    audio: "音频",
    video: "视频",
    entity: "实体",
    aiCreation: "AI 创作",
    scene: "场景",
    selfCreated: "自己创造",
    systemRecommend: "系统推荐",
    teacherManagement: "老师管理",
    studentManagement: "学生管理",
    adminCenter: "管理中心",
    userManagement: "用户管理",
    prefabManagement: "预制体管理",
    tools: "实用工具",
    logout: "退出登录",
    logoutConfirmTitle: "退出登录",
    logoutConfirmMessage: "确定要退出登录吗？",
    logoutConfirmDesc: "退出后需要重新登录才能访问您的账户。",
  },
  breadcrumb: {
    workspace: "主页",
    support: "支持",
    helpCenter: "帮助中心",
    videoTutorial: "视频教程",
    settings: "设置",
    profile: "个人资料",
    accountSecurity: "账号安全",
  },
  ui: {
    moreActions: "更多操作",
    personalSettings: "个人设置",
    user: "用户",
    adminAccount: "管理员账户",
    normalAccount: "普通账户",
    accountRoles: {
      user: "普通账户",
      root: "根账户",
      admin: "超级管理员",
      manager: "管理员",
    },
    fullscreen: "全屏",
    switchTheme: "切换主题",
    language: "语言",
    search: "搜索...",
    selected: "{count}个已选择",
    cancelSelectAll: "取消全选",
    selectAllPage: "全选本页",
    paginationPrev: "上一页",
    paginationNext: "下一页",
    paginationPrefix: "第",
    paginationSuffix: "页 / 共 {total} 页",
    batchDownload: "批量下载",
    batchDelete: "批量删除",
    time: "时间",
    gridView: "网格视图",
    listView: "列表视图",
    resourceDetail: "资源详情",
    save: "保存",
    saveData: "保存数据",
    copy: "复制",
    download: "下载",
    deleteResource: "删除此资源",
    useInEditor: "在编辑器中使用",
    type: "类型",
    format: "格式",
    size: "大小",
    createdAt: "创建时间",
    dimensions: "尺寸",
    resolution: "分辨率",
    duration: "时长",
    modelFaces: "模型面数",
    modelVertices: "模型顶点数",
    preparing: "准备中...",
    filter: "筛选",
    viewInfo: "查看信息",
    importScene: "导入场景",
    dragOrSelectScenePackage: "将场景包拖到此处，或点击选择文件",
    onlyZipScenePackage: "仅支持 .zip 格式的场景包文件",
    importing: "导入中...",
    importSuccess: "导入成功",
    importSuccessSubtitle: "场景已成功导入，可以前往查看",
    importFailed: "导入失败",
    close: "关闭",
    goToView: "前往查看",
    reselectFile: "重新选择文件",
    selectZipScenePackage: "请选择 .zip 格式的场景包文件",
    retryLater: "导入失败，请重试",
    unknownError: "未知错误",
    exportScene: "导出场景",
    exportSuccess: "导出成功",
    exportFailed: "导出失败：{message}",
    batchDownloadDev: "批量下载 {count} 个{resource}（功能开发中）",
    batchDeleteTitle: "批量删除",
    batchDeleteConfirm: "确定要删除选中的 {count} 个{resource}吗？",
    batchDeleteSuccess: "成功删除 {count} 个{resource}",
    cancelDelete: "已取消删除",
    unnamed: "未命名",
    logoutConfirmTitle: "退出登录",
    logoutConfirmMessage: "确定要退出登录吗？",
    logoutConfirmDesc: "退出后需要重新登录才能访问您的账户。",
    transformTitle: "定位物体变换",
    scale: "缩放",
    rotation: "旋转",
    position: "位置",
  },
  web: {
    login: "登录平台",
    nav: {
      home: "首页",
      tutorial: "案例教程",
      authorization: "平台授权",
      forum: "Rokid 论坛",
    },
    hero: {
      loading: "载入中...",
      startCreating: "开始创建",
      authorize: "授权",
      officialPartner: "官方合作伙伴",
      recommendedDevice: "推荐设备",
      visualProgramming: "可视化编程",
      buildArApps: "快速构建AR应用",
      watchDemo: "观看演示",
      scrollDown: "向下滚动了解更多",
      introVideo: "介绍视频",
    },
    news: {
      loadFailed: "加载失败，请稍后再试",
      loadAllFailed: "加载全部内容失败，请稍后再试",
      newest: "最新",
      readMore: "查看详细",
      expandAll: "展开全部",
      collapseAll: "收起",
      articleDetail: "文章详情",
      loadArticleFailed: "加载失败",
      loadArticleFailedDesc: "无法加载文章内容，请稍后再试。",
      close: "关闭",
      share: "分享",
      today: "今天",
      yesterday: "昨天",
      daysAgo: "{n}天前",
      weeksAgo: "{n}周前",
      shareInProgress: "分享功能开发中...",
      newsCategory: "新闻动态",
      tutorialCategory: "案例教程",
      relatedContent: "相关内容",
      videoTutorial: "视频教程内容",
    },
    buy: {
      title: "如何获得平台授权",
      subtitle: "只需从本渠道购买相应设备，即可终生使用平台。",
      freeVersion: "免费版本",
      freeVersionDesc: "无需任何费用，即可使用！",
      deviceUnlock: "设备解锁",
      deviceUnlockDesc: "从本站渠道采购硬件设备，获得完全授权去除水印。",
      customDev: "定制开发",
      customDevDesc: "定制开发项目，可免费解锁十台设备",
      unlockCode: "设备解锁",
      unlockCodeDesc: "可以单独购买设备解锁码，解锁设备",
      scanToBuy: "微信扫描购买",
      buyNow: "立即购买",
      cancel: "取消",
      contactUnlock: "购买完成后请联系解锁设备",
      scanQrCode: "请扫小程序码进入商城",
      purchase: "购买",
      confirm: "确认",
    },
  },
};
