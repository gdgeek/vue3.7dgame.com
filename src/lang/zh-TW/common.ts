// common translations for zh-TW
export default {
  common: {
    appUpdate: {
      available:
        "網站有新版本。請先保存或匯出未保存內容、確認待處理操作並關閉執行場景，再手動重新整理；之後重新發現 WebMCP 工具。",
      dismiss: "稍後",
    },
    recovery: {
      summary: "保存衝突：本地修改仍未保存。",
      open: "查看衝突與保留副本",
      boundary:
        "這裡保留的是被伺服器拒絕的提交副本，可能不含之後的編輯。預覽最多顯示 12,000 字元，匯出包含完整副本。讀取伺服器不會覆蓋編輯器或更新保存基線；請先匯出，再核對最新版並重新編輯。",
      export: "匯出被拒絕的提交",
      read: "唯讀查看伺服器最新版",
      keep: "保留本地並繼續編輯",
      local: "本次提交副本",
      server: "伺服器目前內容（唯讀）",
      failed: "讀取失敗或權限已變化，未顯示伺服器內容。",
    },
    publicationHistory: {
      retention:
        "最多保留最近 {count} 份發佈正文，超出後自動清理舊正文；目前發佈不受影響。",
      expired:
        "該版本正文已按保留策略清理。請重新整理列表選擇仍保留的版本，無需再次發佈。",
      from: "比較起始版本",
      to: "比較目標版本",
      compare: "比較發布版本",
      identical: "原始正文完全相同",
      different: "原始正文不同；以下為結構差異摘要",
      truncated: "差異達到上限，結果不完整；請分別匯出核對。",
      export: "匯出正文與資源引用",
      pending:
        "發佈已成功，歷史正文尚未核驗。可在發佈歷史中重試讀取，請勿重複發佈。",
      title: "發佈歷史",
      boundary:
        "服务器保存的固定发布版本，与本机草稿独立。查看历史不会覆盖当前场景。",
      refresh: "重新整理列表",
      capacity: "{count} 个版本 · 已用 {used} / {budget}",
      capacityWarning: "归档容量接近上限，请联系管理员规划容量。",
      empty: "暂无固定发布历史。旧发布不会被自动补录为历史原件。",
      date: "发布时间",
      language: "语言",
      actor: "发布人 ID",
      version: "版本",
      inspect: "读取并核验",
      more: "加载更多",
      detail: "版本详情",
      verified: "歷史正文核驗通過",
      resources:
        "只核验已归档正文；资源文件未复制，可用性尚未检查，也不保证能恢复编辑工程。",
      body: "查看固定正文",
      forbidden: "当前账号没有读取此场景历史的权限。",
      failed:
        "读取或核验未通过。可重试读取；不会再次发布或用当前快照代替历史。",
    },
    editorLoading: {
      progressLabel: "資源載入進度",
      count: "已完成 {completed} / {total} 項",
      workFailed: "載入未完成，請重新載入。",
      stages: {
        connecting: "正在連接編輯器",
        data: "正在讀取場景／實體資料",
        initializing: "正在初始化編輯器",
        assets: "正在載入資源",
        finishing: "正在整理場景與編輯狀態",
        ready: "載入完成",
        error: "載入失敗",
        model: "正在載入模型",
        module: "正在載入場景實體",
        anchor: "正在載入錨點",
        space: "正在載入空間背景",
      },
      message: "編輯器正在載入…",
      hint: "載入完成後即可編輯、儲存、執行或開啟腳本。",
      failed: "編輯器載入未完成，請重試。",
      retry: "重新載入",
    },
    confirm: "確認",
    cancel: "取消",
    selectUser: "選擇用戶",
    username: "用戶名",
    nickname: "暱稱",
    select: "選擇",
    noUsers: "未找到用戶",
    noData: "暫無數據",
    enter: "進入",
    open: "打開",
    add: "添加",
    name: "名稱",
    description: "描述",
    edit: "編輯",
    rename: "重新命名",
    back: "返回",
    updateSuccess: "更新成功",
    createSuccess: "創建成功",
    createFailed: "創建失敗",
    delete: "刪除",
    deleteSuccess: "刪除成功",
    deleteFailed: "刪除失敗",
    operationFailed: "操作失敗",
    networkError: "網絡錯誤，請檢查您的網絡連接",
    updateFailed: "更新失敗",
    invalidData: "數據無效",
    button: {
      save: "保存",
    },
    scriptSaveConfirm: {
      title: "提示",
      message: "是否保存當前腳本",
      confirm: "是",
      cancel: "否",
    },
    sceneSaveConfirm: {
      title: "提示",
      message: "是否保存當前場景",
      confirm: "是",
      cancel: "否",
    },
    entitySaveConfirm: {
      title: "提示",
      message: "是否保存當前實體",
      confirm: "是",
      cancel: "否",
    },
    scriptDraft: {
      title: "版本管理",
      autoSave: "自動保存",
      interval: "保存間隔",
      minutes: "分鐘",
      statusSaving: "保存中",
      statusSaved: "已保存",
      statusAutoSaved: "已自動保存",
      statusDirty: "待保存",
      versionList: "歷史版本",
      empty: "暫無歷史版本",
      clearHistory: "清空歷史版本",
      restore: "回滾到此版本",
      restoreSuccess: "已恢復到選中版本，請確認後保存",
      autoSavedNotice: "已自動保存",
      autoTag: "自動保存",
      manualTag: "手動保存",
      emptySummary: "暫無內容",
      summaryAdded: "新增 {items}",
      summaryRemoved: "刪除 {items}",
      summaryModified: "修改 {items}",
      entry: "版本管理",
    },
    unityPreview: {
      entry: "運行場景",
      title: "運行場景",
      helpClick: "先點擊運行畫面，使其取得焦點",
      helpRotate: "按住 Alt（Mac：⌥ Option）並用滑鼠左鍵拖曳，旋轉視角",
      helpZoomPan: "滾輪或觸控板雙指捲動可縮放；滑鼠右鍵拖曳可平移",
      helpFullscreen: "全螢幕可獲得更完整預覽",
      fullscreen: "全螢幕運行場景",
      close: "關閉運行場景",
    },
    message: {
      createSuccess: "創建成功",
      saveSuccess: "保存成功",
      saveFailed: "保存失敗",
    },
  },
  errorPage: {
    back: "返回",
    gifSource: "圖片來源於",
    page: "Oops! 頁面出錯了",
    noPermission: "你沒有權限訪問該頁面",
    contactLeader: "請聯繫你的上級或管理員",
    orGoTo: "或者去",
    backHome: "回到首頁",
    justLook: "隨便逛逛",
    clickToSee: "點擊查看",
    justLookTitle: "隨便逛逛",
  },
  sidebar: {
    home: "首頁",
    resources: "素材庫",
    model: "模型",
    voxel: "體素",
    picture: "圖片",
    audio: "音頻",
    video: "視頻",
    entity: "實體",
    aiCreation: "AI 創作",
    scene: "場景",
    selfCreated: "我的創作",
    systemRecommend: "系統推薦",
    teacherManagement: "老師管理",
    studentManagement: "學生管理",
    adminCenter: "管理中心",
    userManagement: "用戶管理",
    prefabManagement: "預製體管理",
    tools: "實用工具",
    logout: "登出",
    logoutConfirmTitle: "登出",
    logoutConfirmMessage: "確定要登出嗎？",
    logoutConfirmDesc: "登出後需要重新登入才能存取您的帳戶。",
  },
  breadcrumb: {
    workspace: "主頁",
    support: "支援",
    helpCenter: "幫助中心",
    videoTutorial: "影片教學",
    settings: "設定",
    profile: "個人資料",
    accountSecurity: "帳號安全",
  },
  ui: {
    moreActions: "更多操作",
    personalSettings: "個人設定",
    user: "用戶",
    adminAccount: "管理員帳戶",
    normalAccount: "普通帳戶",
    accountRoles: {
      user: "普通帳戶",
      root: "根帳戶",
      admin: "超級管理員",
      manager: "管理員",
    },
    search: "搜尋...",
    selected: "已選擇 {count} 個",
    cancelSelectAll: "取消全選",
    selectAllPage: "全選本頁",
    paginationPrev: "上一頁",
    paginationNext: "下一頁",
    paginationPrefix: "第",
    paginationSuffix: "頁 / 共 {total} 頁",
    batchDownload: "批次下載",
    batchDelete: "批次刪除",
    time: "時間",
    type: "類型",
    format: "格式",
    size: "大小",
    createdAt: "建立時間",
    dimensions: "尺寸",
    resolution: "解析度",
    duration: "時長",
    modelFaces: "模型面數",
    modelVertices: "模型頂點數",
    gridView: "網格視圖",
    listView: "列表視圖",
    resourceDetail: "資源詳情",
    viewInfo: "查看資訊",
    save: "保存",
    saveData: "保存資料",
    copy: "複製",
    download: "下載",
    deleteResource: "刪除此資源",
    useInEditor: "在編輯器中使用",
    importScene: "匯入場景",
    dragOrSelectScenePackage: "將場景包拖曳到此處，或點擊選擇文件",
    onlyZipScenePackage: "僅支援 .zip 格式的場景包文件",
    importing: "匯入中...",
    preparing: "準備中...",
    importSuccess: "匯入成功",
    importSuccessSubtitle: "場景已成功匯入，可以前往查看",
    importFailed: "匯入失敗",
    close: "關閉",
    goToView: "前往查看",
    reselectFile: "重新選擇文件",
    selectZipScenePackage: "請選擇 .zip 格式的場景包文件",
    retryLater: "匯入失敗，請重試",
    unknownError: "未知錯誤",
    exportScene: "匯出場景",
    exportSuccess: "匯出成功",
    exportFailed: "匯出失敗：{message}",
    batchDownloadDev: "批次下載 {count} 個{resource}（功能開發中）",
    batchDeleteTitle: "批次刪除",
    batchDeleteConfirm: "確定要刪除選中的 {count} 個{resource}嗎？",
    batchDeleteSuccess: "成功刪除 {count} 個{resource}",
    cancelDelete: "已取消刪除",
    unnamed: "未命名",
    filter: "篩選",
    fullscreen: "全螢幕",
    switchTheme: "切換主題",
    language: "語言",
    logoutConfirmTitle: "登出",
    logoutConfirmMessage: "確定要登出嗎？",
    logoutConfirmDesc: "登出後需要重新登入才能存取您的帳戶。",
    transformTitle: "定位物件變換",
    scale: "縮放",
    rotation: "旋轉",
    position: "位置",
  },
  emailVerification: {
    codeSent: "驗證碼已發送到您的郵箱",
    verifySuccess: "郵箱驗證成功",
    sentToCurrentEmail: "驗證碼已發送到當前綁定郵箱",
    oldEmailVerified: "舊郵箱驗證成功，請繼續完成新郵箱綁定",
    unbindSuccess: "郵箱解綁成功",
  },
  request: {
    loginExpired: "登錄過期，請重新登錄",
    networkError: "網絡錯誤，請檢查您的網絡連接",
    serverError: "服務器內部錯誤，請稍後再試",
    error404: "請求失敗，未找到資源 (404)",
    unknownError: "未知錯誤",
  },
  web: {
    login: "登錄平台",
    nav: {
      home: "首頁",
      tutorial: "案例教程",
      authorization: "平台授權",
      forum: "Rokid 論壇",
    },
    hero: {
      loading: "載入中...",
      startCreating: "開始創建",
      authorize: "授權",
      officialPartner: "官方合作夥伴",
      recommendedDevice: "推薦設備",
      visualProgramming: "視覺化編程",
      buildArApps: "快速構建AR應用",
      watchDemo: "觀看演示",
      scrollDown: "向下滾動了解更多",
      introVideo: "介紹視頻",
    },
    news: {
      loadFailed: "載入失敗，請稍後再試",
      loadAllFailed: "載入全部內容失敗，請稍後再試",
      newest: "最新",
      readMore: "查看詳細",
      expandAll: "展開全部",
      collapseAll: "收起",
      articleDetail: "文章詳情",
      loadArticleFailed: "載入失敗",
      loadArticleFailedDesc: "無法載入文章內容，請稍後再試。",
      close: "關閉",
      share: "分享",
      today: "今天",
      yesterday: "昨天",
      daysAgo: "{n}天前",
      weeksAgo: "{n}週前",
      shareInProgress: "分享功能開發中...",
      newsCategory: "新聞動態",
      tutorialCategory: "案例教程",
      relatedContent: "相關內容",
      videoTutorial: "視頻教程內容",
    },
    buy: {
      title: "如何獲得平台授權",
      subtitle: "只需從本渠道購買相應設備，即可終生使用平台。",
      freeVersion: "免費版本",
      freeVersionDesc: "無需任何費用，即可使用！",
      deviceUnlock: "設備解鎖",
      deviceUnlockDesc: "從本站渠道採購硬體設備，獲得完全授權去除浮水印。",
      customDev: "定制開發",
      customDevDesc: "定制開發項目，可免費解鎖十台設備",
      unlockCode: "設備解鎖",
      unlockCodeDesc: "可以單獨購買設備解鎖碼，解鎖設備",
      scanToBuy: "微信掃描購買",
      buyNow: "立即購買",
      cancel: "取消",
      contactUnlock: "購買完成後請聯繫解鎖設備",
      scanQrCode: "請掃小程式碼進入商城",
      purchase: "購買",
      confirm: "確認",
    },
  },
};
