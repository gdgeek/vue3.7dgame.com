...
- 发布成功返回 snapshotId、publicationRevision、contentHash，再通过独立 GET 读取对应不可变快照。目标、版本和哈希匹配且返回快照正文后，才标记 `readBackVerified: true`。页面刷新失败保留已确认的发布回执并返回 refreshWarning。

后端配套提供保存/脚本保存/发布的 UUID 幂等键和 `If-Match` 乐观锁。编辑器人工保存也使用加载时的服务器版本，409 后保留本地内容，不自动覆盖新版本。缺少服务器版本或匹配回执时不能视为保存成功。后端 schema 和路由必须先部署，详细迁移见后端 `docs/webmcp-p1.md`。
...
场景上下文的 published 来自独立后端 publication 查询。查询失败时保留未知，不猜测旧 verseRelease 字段。`xrugc_get_scene_publication` 可读取当前发布状态或指定归档版本；迁移前旧快照没有不可变发布版本，不会被补记成已验证发布。
