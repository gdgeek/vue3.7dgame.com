...
# 排除编号变量（APP_API_N_* 和 APP_DOMAIN_N_*）和 APP_RESOLVER，只替换其余 APP_ 变量
VARS=$(env | grep '^APP_' | grep -v '^APP_API_[0-9]' | grep -v '^APP_DOMAIN_[0-9]' | grep -v '^APP_RESOLVER' | sed 's/=.*//' | sed 's/^/\$/' | tr '\n' ' ')
if [ -n "$VARS" ]; then
