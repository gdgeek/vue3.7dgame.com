...
## 当前验收：2026-09-12 实体脚本抽屉

- 本次复用场景脚本抽屉，实体页的顶部按钮和编辑器内「脚本 → 脚本编辑」菜单都打开右侧抽屉。共享标题栏保存／版本历史按钮组，以及逻辑编辑／代码查看标签页。
- 本地 Chrome 验收地址：`http://127.0.0.1:3001/meta/scene?id=3642&lang=zh-CN&theme=modern-blue`，实体「鸡蛋花」，modern-blue 主题。此本地服务连接正式 API，实体编号与用户提供的开发环境 809 不混用。
- 截图证据：本任务 CUA 内联截图「检查实体脚本抽屉样式与加载」。截图包含浏览器外框，1245 × 768 px；未导出独立图片，也未做不同像素密度下的精确对照。
- 实测通过：两个入口均打开抽屉；完整 Blockly 工作区正常显示；Lua 和 JavaScript 标签均显示已有代码；标题栏版本历史能打开并关闭；关闭主抽屉后原实体编辑器仍可见，页面 URL 与实体 iframe 地址保持一致；再次打开正常加载。
- 自动化回归覆盖未保存快照、未保存新增资源补齐与已删除引用剔除、关闭取消、保存回执的实体 ID／版本匹配、旧请求失效及实体／脚本工具切换。独立检出中完整回归 403 个文件、5400 项通过、3 项跳过，覆盖率 83.4%；完整生产构建通过。
- 未在浏览器中执行保存、版本恢复或发布；真实保存回执、手动修改后的相机状态、窄屏与暗色主题仍未实测。WebMCP 生命周期本轮以自动化测试验证，未声称完成真实浏览器工具调用验收。
- 当前改动在本地，开发站是否可见以推送和部署结果为准。

本轮结果：上述浏览器只读流程通过，持久化操作未实测。

## 此前验收：2026-09-12 抽屉工具栏精简
